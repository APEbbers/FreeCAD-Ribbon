...
class StyleMapping_Ribbon:

    darkMode = None

    def ReturnStyleItem(self, ControlName, ShowCustomIcon=False, IgnoreOverlay=False):
        """
        ...
        IsInList = False
        for key, value in self.self.StyleMapping_default["Stylesheets"].items():
        ...
        try:
            if ControlName == "TitleBarButtons":
                return self.StyleMapping["Stylesheets"][ControlName]
            if isIcon is True:
                result = None
                PixmapName = ""
                if Parameters_Ribbon.CUSTOM_ICONS_ENABLED is True or ShowCustomIcon is True:
                    PixmapName = self.StyleMapping["Stylesheets"][ControlName]
                else:
                    PixmapName = ""
                if PixmapName == "" or PixmapName is None:
                    PixmapName = self.StyleMapping_default["Stylesheets"][currentStyleSheet][ControlName]
                    if PixmapName == "" or PixmapName is None:
                        PixmapName = self.StyleMapping_default["Stylesheets"][""][ControlName]
            ...
            if isIcon is False:
                ...
                if Parameters_Ribbon.CUSTOM_COLORS_ENABLED is True:
                    result = self.StyleMapping["Stylesheets"][ControlName]
                if (
                ...
                if result == "" or result is None:
                    result = self.StyleMapping_default["Stylesheets"][currentStyleSheet][ControlName]
                    if result == "" or result is None:
                        result = self.StyleMapping_default["Stylesheets"][""][ControlName]
                return result
        except Exception as e:
            print(e)
            return None

    def ReturnStyleSheet(self, control, radius="2px", padding_right="0px", padding_bottom="0px", width="16px"):
        """
        Enter one of the names below:
        ...
        StyleSheet = ""
        try:
            BorderColor = self.ReturnStyleItem("Border_Color")
            BackgroundColor = self.ReturnStyleItem("Background_Color")
            ApplicationButton = self.ReturnStyleItem("ApplicationButton_Background")
            HoverColor = self.ReturnStyleItem("Background_Color_Hover")
            FontColor = self.ReturnStyleItem("FontColor")
    ...
    def GetIconBasedOnTag(self, ControlName=""):
        iconSet = {}
        iconName = ""
        IsDarkTheme = self.darkMode
